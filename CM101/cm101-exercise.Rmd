...
square <- function(x) {
  y <- x^2
  return(y)
  5  #won't print because return(y) comes first, good for ending early
}
...
```{r}
test_that("Simple cases work", {   #we don't actually need test_that function
...
mmm2 <- function(x) {
    if (!is.numeric(x)) {
        stop("Argument must be numeric")  #paste("Expecting x to be numeric. You gave me ", typeof(x))
    }
    max(x) - min(x)
}
...
```{r}
mmm1((numeric(0))) #still no, need more work to get non-positive vector error message
mmm1(gapminder)
mmm1(gapminder$country)
...
```{r}
#Doesn't need to be called x for the input, x is a dummy variable within the function only
x <- 1:10
mmm2(x)
```
...
Aside, quantiles.
0.9 quantile is 90% of the time output will be below (beat 10% of the time)
...
```{r}
qd2 <- function(x, probs=c(0,1), na.rm) {
    stopifnot(is.numeric(x))
    if (length(x) == 0) {
        warning("You inputted a length-0 x. Expecting length >=1. Returning NULL.")
        return(NULL)
    } 
    qvec <- quantile(x, probs, na.rm)
    max(qvec) - min(qvec)
}
```
...
```{r}
qd2 <- function(x, probs=c(0,1), na.rm=FALSE, type) {
    stopifnot(is.numeric(x))
    if (length(x) == 0) {
        warning("You inputted a length-0 x. Expecting length >=1. Returning NULL.")
        return(NULL)
